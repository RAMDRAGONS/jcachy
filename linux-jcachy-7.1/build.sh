#!/bin/bash
# JCachy build and installation script.
...
if [[ "$#" -gt 0 && "$1" =~ ^(-l|--list|list)$ ]]; then
  ...
  confs=$( ls -1dvr ~/.config/jcachy/*.conf 2>/dev/null )
  if [ -z "$confs" ]; then
    msg="Zero JCachy configs. Run the 'confmod.sh' utility to make one."
    echo -e "$msg\n"
  else
...
if [ "$#" -gt 0 ]; then
  ...
  if [ ! -e ~/.config/jcachy/$conf.conf ]; then
    echo -e "${RED}ERROR:${NC} '~/.config/jcachy/$conf.conf' does not exist."
    echo -e "Run the 'confmod.sh' utility and try again."
    echo
    exit 1
  fi

  source ~/.config/jcachy/$conf.conf
  ...
  : ${_buildtype:=clang}
  : ${_autofdo:=no}
  : ${_autofdo_profile_name:=jcachy.afdo}
  : ${_hugepage:=always}
  : ${_kernel_suffix:=}
  : ${_localmodcfg:=no}
...
export _preempt _processor_opt _build_debug
...
# Build and install the JCachy kernel.
